...
class NodeManageController extends React.Component {
  ...
  onCustomNameChange(name) {
    ...
    const nameState =
      name.trim().length <= 4
        ? 'EXCEPTION.ADD_NODE.NAME'
        : Object.values(nodes).some((node) => node.name === name.trim())
        ? 'EXCEPTION.ADD_NODE.REPEAT_NAME'
  ...
  addCustomNode() {
    ...
    app.getNodes();
    Toast.success(
      formatMessage({ id: 'SETTING.SUCCESS.ADD_NODE' }),
      3,
      () => {
  ...
  render() {
    ...
    return (
      <div className='insetContainer nodeManage'>
        ...
        {showAddNodeDialog ? (
          <div
            className='addNodeWrap'
            onClick={() => this.setState({ showAddNodeDialog: false })}
          >
            <div className='addNode' onClick={(e) => e.stopPropagation()}>
              ...
              <div className='input'>
                <input
                  ...
                  placeholder={formatMessage({
                    id: 'SETTINGS.CUSTOM_NODE.NAME.PLACEHOLDER',
                  })}
                  onChange={(e) => this.onCustomNameChange(e.target.value)}
                />
              </div>
              {!isValid &&
              name.state !== VALIDATION_STATE.VALID &&
              name.state !== VALIDATION_STATE.NONE ? (
              ...
              <div className='input'>
                <input
                  ...
                  placeholder={formatMessage({
                    id: 'SETTINGS.CUSTOM_NODE.FULL_NODE.PLACEHOLDER',
                  })}
                  onChange={(e) =>
                    this.onCustomNodeChange('fullNode', e.target.value)
                  }
                />
              </div>
              {!isValid && fullNode.state === VALIDATION_STATE.INVALID ? (
                <div className='tipError'>
                  <FormattedMessage id='EXCEPTION.ADD_NODE.NODE_URL' />
                </div>
              ) : null}
              <label>
              ...
              <div className='input'>
                <input
                  ...
                  placeholder={formatMessage({
                    id: 'SETTINGS.CUSTOM_NODE.SOLIDITY_NODE.PLACEHOLDER',
                  })}
                  onChange={(e) =>
                    this.onCustomNodeChange('solidityNode', e.target.value)
                  }
                />
              </div>
              {!isValid && solidityNode.state === VALIDATION_STATE.INVALID ? (
                <div className='tipError'>
                  <FormattedMessage id='EXCEPTION.ADD_NODE.NODE_URL' />
                </div>
              ) : null}
              ...
              <div className='input'>
                <input
                  ...
                  placeholder={formatMessage({
                    id: 'SETTINGS.CUSTOM_NODE.EVENT_SERVER.PLACEHOLDER',
                  })}
                  onChange={(e) =>
                    this.onCustomNodeChange('eventServer', e.target.value)
                  }
                />
              </div>
              {!isValid && eventServer.state === VALIDATION_STATE.INVALID ? (
                <div className='tipError'>
                  <FormattedMessage id='EXCEPTION.ADD_NODE.NODE_URL' />
                </div>
              ) : null}
        ...
        <div className='pageHeader'>
          ...
          <div className='chains'>
            ...
            <div className='chainWrap'>
              {Object.entries(chains.chains).map(([chainId, chain]) => (
                <div
                  className='item'
                  onClick={() => this.setState({ selectedChain: chainId })}
                >
                  {chain.name}
                </div>
              ))}
        ...
        <div className='greyModal'>
          <div className='node scroll'>
            {Object.entries(nodes.nodes)
              .filter(([nodeId, node]) => node.chain === selectedChain)
              .map(
                ([
                  nodeId,
                  {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`r1${
                          nodeId === nodes.selected ? ' selected' : ''
                        }`}
                      >
                        {name}
                        {isDelete ? (
          ...
          {selectedChain === '_' ? (
            <div
              className='addNode'
              onClick={() => this.setState({ showAddNodeDialog: true })}
            >
