...
import {
  getBankDefaultDataApi,
  getBankIsRentApi,
  getBankBalanceEnoughApi,
  postBankOrderApi,
} from '@tronlink/popup/src/fetch/tronLending/tronLending';
...
class BankController extends React.Component {
  ...
  async defaultDataFun() {
    ...
    if (Number.isFinite(defaultData.energy))
      costTrx = Math.ceil(
        (defaultData.energy / totalenergyLimitNum) * totalEnergyWeight
      );
    else costTrx = 0;
    this.setState({
  ...
  async isValidRentAddress() {
    ...
    if (curaAddress === '') address = selectedaAddress;
    else address = curaAddress;
    const requestUrl = getBankIsRentApi();
    const isRentDetail = await PopupAPI.isValidOrderAddress(
      address,
      requestUrl
    );
    const recipient = {
  ...
  async handlerRentNumChange(e, _type) {
    ...
    if (!rentVal.length) return this.setState({ rentNum });
    if (!Utils.validatInteger(rentVal)) {
      ...
      const totalenergyLimitNum = selected.TotalEnergyLimit;
      // predict num energy
      if (Number.isFinite(totalEnergyWeight))
        rentNum.predictVal = Math.ceil(
          (rentVal / totalEnergyWeight) * totalenergyLimitNum
        );
      ...
      if (rentVal <= rentNumMax && rentVal >= rentNumMin) {
        if (_type === 2) {
          ...
          const isValid = await PopupAPI.isValidOverTotal(
            address,
            rentVal * Math.pow(10, 6),
            requestUrl
          );
  ...
  rentDealSendFun(e) {
    ...
    if (recipient.value === '') recipientAddress = address;
    else recipientAddress = recipient.value;
    if (submitBtnIsClick) {
      this.setState({
      ...
      hashResult
        .then((res) => {
          const successRes = PopupAPI.bankOrderNotice(
            recipientAddress,
            res,
            requestUrl
          );
  ...
  render() {
    ...
    let recipientVal;
    if (recipient.value === '') recipientVal = selected.address;
    else recipientVal = recipient.value;
    const orderList = [
      { id: 'BANK.RENTINFO.PAYADDRESS', user: 1, value: selected.address },
      { id: 'BANK.RENTINFO.RECEIVEADDRESS', user: 1, value: recipientVal },
      { id: 'BANK.RENTINFO.RENTNUM', tip: 1, value: `${rentNum.value}WELUPS` },
      { id: 'BANK.RENTINFO.RENTDAY', type: 3, value: rentDay.value },
      { id: 'BANK.RENTINFO.PAYNUM', type: 0, value: `${rentUnit.cost}WELUPS` },
    ];
    const saveCost = parseFloat((rentUnit.cost / discount) * (1 - discount));
    const myImg = (src) => {
      return require(`../../assets/images/new/tronBank/${src}.svg`);
    };
    return (
      <div
        className='TronBankContainer'
        onClick={(e) => {
          ...
          <div
            className={
              this.state.popoverVisible
                ? 'dropList menuList menuVisible'
                : 'dropList menuList'
            }
          >
            <div
              onClick={() => {
                PopupAPI.changeState(APP_STATE.TRONBANK_RECORD);
              }}
              className='item'
            >
              <img
            ...
            <div
              onClick={(e) => {
                PopupAPI.changeState(APP_STATE.TRONBANK_HELP);
              }}
              className='item'
            >
              <img
          ...
          <div className='bankContent'>
            ...
            <div className='accountContent'>
              <section className='accountInfo infoSec'>
                <label>
                  <FormattedMessage id='BANK.SEND.PAY_ACCOUNT' />
                </label>
                <div className='selectedAccount'>
                  {selected.name.length > 6
                    ? `${selected.name.slice(0, 6)}…`
                    : selected.name}{' '}
                  <span>{selected.address}</span>
                </div>
                <div className='balance'>
                  <FormattedMessage
                    id='BANK.INDEX.BALANCE'
                    values={{ amount: selected.balance / Math.pow(10, 6) }}
                  />
                </div>
              </section>
              <section className='infoSec'>
                <label>
                  <FormattedMessage id='BANK.SEND.RECEIVE_ADDRESS' />
                </label>
                <div
                  className={
                    recipient.error ||
                    !validOrderOverLimit.valid ||
                    isOnlineAddress.error
                      ? 'receiveAccount errorBorder'
                      : 'receiveAccount normalBorder'
                  }
                >
                  <input
                    ref={(rentAddressInput) =>
                      (this.rentAddressInput = rentAddressInput)
                    }
                    onChange={(e) => {
                      this.onRecipientChange(e, 1);
                    }}
                    onBlur={(e) => this.onRecipientChange(e, 2)}
                    placeholder={formatMessage({
                      id: 'BANK.INDEX.PLACEHOLDER',
                      values: { min: rentNumMin },
                    })}
                  />
                </div>
                {recipient.error ? (
                ...
                {curentInputBalance.show ? (
                  <div className='balance'>
                    <FormattedMessage
                      id='BANK.INDEX.USED'
                      values={{ used: curentInputBalance.used }}
                    />
                    /
                    <FormattedMessage
                      id='BANK.INDEX.TOTAL'
                      values={{ total: curentInputBalance.total }}
                    />
                  </div>
                ) : null}
              </section>
            ...
            <div className='rentContent'>
              <section className='infoSec'>
                ...
                <div
                  className={
                    ...
                    accountMaxBalance.valid
                      ? 'rentNumWrapper errorBorder'
                      : 'rentNumWrapper normalBorder'
                  }
                >
                  <input
                    value={rentNum.value}
                    onChange={(e) => {
                      this.handlerRentNumChange(e, 1);
                    }}
                    onBlur={(e) => this.handlerRentNumChange(e, 2)}
                    className='commonInput rentNumInput'
                    placeholder={
                      formatMessage({ id: 'BANK.INDEX.FREEZEPLACEHOLDER' }) +
                      `（${rentNumMin}-${rentNumMax}）`
                    }
                  />
                  <span>WELUPS</span>
                </div>
                {rentNum.formatError ? (
                  <div className='errorMsg'>
                    <FormattedMessage
                      id='BANK.INDEX.RENTNUMFORMATERROR'
                      values={{ min: rentNumMin, max: rentNumMax }}
                    />
                  </div>
                ) : null}
                {rentNum.error ? (
                ...
                {rentNum.predictStatus ? (
                  <div className='predictMsg'>
                    <FormattedMessage
                      id='BANK.INDEX.FORECASTNUM'
                      values={{ num: rentNum.predictVal }}
                    />
                  </div>
                ) : null}
                {accountMaxBalance.valid ? (
              ...
              <section className='infoSec'>
                <label>
                  <FormattedMessage
                    id='BANK.INDEX.RENTDAY'
                    values={{ min: rentDayMin, max: rentDayMax }}
                  />
                </label>
                <div
                  className={
                    rentDay.error || rentDay.formatError
                      ? 'dayRange errorBorder'
                      : 'dayRange normalBorder'
                  }
                >
                  <span
                    className={
                    ...
                    onClick={(e) => this.handlerRentDayFun(1)}
                  >
                    <Button
                      className='operatingBtn'
                      icon={
                        <img
                          className='operationReduceIcon'
                          src={myImg('subtrac')}
                          alt='subtrac'
                        />
                      }
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      this.handlerRentDayChange(e, 1);
                    }}
                    onBlur={(e) => {
                      this.handlerRentDayChange(e, 2);
                    }}
                    className='commonInput rentDay'
                    placeholder={
                      formatMessage({ id: 'BANK.INDEX.RENTPLACEHOLDER' }) +
                      `(${rentDayMin}-${rentDayMax})`
                    }
                    type='text'
                  />
                  <span
                    ...
                    onClick={(e) => this.handlerRentDayFun(2)}
                  >
                    <Button
                      className='operatingBtn'
                      icon={
                        <img
                          className='operationAddIcon'
                          src={myImg('add')}
                          alt='add'
                        />
                      }
                ...
                {rentDay.formatError ? (
                  <div className='errorMsg rentError'>
                    <FormattedMessage
                      id='BANK.INDEX.RENTDAYFORMATERROR'
                      values={{ min: rentDayMin, max: rentDayMax }}
                    />
                  </div>
                ) : null}
              ...
              {rentNum.valid && rentDay.valid ? (
                <section className='calculation'>
                  <div className='info'>
                    <span>
                      {rentNum.value}WELUPS*{rentDay.value}
                    </span>
                    <span className='numInfo'>
                      {rentDay.value > 2 ? (
                        <FormattedMessage id='BANK.INDEX.RENTDAYUNITS' />
                      ) : (
                        <FormattedMessage id='BANK.INDEX.RENTDAYUNIT' />
                      )}
                    </span>
                    <span className='pointColor'>
                      <FormattedMessage id='BANK.INDEX.RENTCONST' />{' '}
                      {rentUnit.cost} WELUPS
                    </span>
                  </div>
                  <div className='curNum'>
                    {language === 'en' ? (
                      <span>
                        (
                        <span className='pointColor'>
                          {saveCost.toFixed(2)}WELUPS{' '}
                        </span>
                        saved than burn-WELUPS,
                        <span className='pointColor'>
                          {rentNum.value}WELUPS
                        </span>{' '}
                        required to freeze)
                      </span>
                    ) : (
                      <span>
                        ...
                        <span className='pointColor'>
                          <FormattedMessage id='BANK.INDEX.ESTIMATESAVE' />
                          {saveCost.toFixed(2)}trx
                        </span>
                        ,<FormattedMessage id='BANK.INDEX.ESTIMATEINFO' />
                        <span className='pointColor'>
                          {rentNum.value}WELUPS
                        </span>
                        )
                ...
                <section className='rentIntroduce'>
                  <div className='info'>
                    {language === 'en' ? (
                      <span>
                        renting {defaultUnit.num * 10}k energy *{' '}
                        {defaultUnit.day} day costs {defaultUnit.cost}WELUPS
                      </span>
                    ) : (
                      <FormattedMessage
                        id='BANK.INDEX.RENTINTRODUCE'
                        values={{ ...defaultUnit }}
                      />
                    )}
                  </div>
            ...
            <Button
              disabled={
                recipient.valid && rentNum.valid && rentDay.valid ? false : true
              }
              className={
                recipient.valid && rentNum.valid && rentDay.valid
                  ? 'bankSubmit normalValid'
                  : 'bankSubmit inValid'
              }
        ...
        >
          <div className='rentIntroduceCont'>
            ...
            <Button
              ...
              onClick={() => {
                this.onModalClose('rentModalVisible')();
              }}
              size='small'
            >
              <FormattedMessage id='BANK.RENTNUMMODAL.BUTTON' />
            </Button>
          </div>
        </Modal>
        <Modal
        ...
        >
          <div className='rentIntroduceCont'>
                ...
                {orderList.map((val, key) => (
                  <div key={key} className='orderList'>
                    ...
                    <span className='orderStatus'>
                      {val.user === 1
                        ? `${val.value.substr(0, 6)}...${val.value.substr(-6)}`
                        : val.value}
                      {val.tip === 1 ? (
                        <FormattedMessage
                          id='BANK.RENTINFO.TIPS'
                          values={{ num: rentNum.predictVal }}
                        />
                      ) : null}
                      {val.type === 3 ? (
                        <FormattedMessage id='BANK.RENTRECORD.TIMEUNIT' />
                      ) : null}
                    </span>
                  </div>
                ))}
            ...
            <section className='operateBtn'>
              <Button
                className='modalCloseBtn confirmClose'
                onClick={() => {
                  this.onModalClose('rentConfirmVisible')();
                }}
              >
                <FormattedMessage id='BANK.RENTINFO.CANCELBTN' />
              </Button>
              {submitBtnIsClick ? (
                <Button
                  className='modalPayBtn'
                  onClick={(e) => {
                    this.rentDealSendFun(e);
                  }}
                >
                  <FormattedMessage id='BANK.RENTINFO.PAYBTN' />
                </Button>
              ) : (
                <Button className='modalPayBtn modalPayDisabled'>
                  <FormattedMessage id='BANK.RENTINFO.PAYBTN' />
                </Button>
              )}
            </section>
