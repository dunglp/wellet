...
class BankDetailController extends React.Component {
  ...
  componentDidMount() {
    ...
    let requestId;
    if (selected.selectedBankRecordId)
      requestId = selected.selectedBankRecordId;
  ...
  async getBankRecordDetail(_id) {
    ...
    const orderList = [
      {
        id: 'BANK.RENTDETAIL.RENTNUM',
        type: 0,
        value: `${recordDetail.freeze_amount / Math.pow(10, 6)}WELUPS`,
      },
      { id: 'BANK.RENTDETAIL.RENTTIME', type: 2, value: recordDetail.days },
      {
        id: 'BANK.RENTDETAIL.PAYNUM',
        type: 0,
        value: `${recordDetail.pay_amount / Math.pow(10, 6)}WELUPS`,
      },
      {
        id: 'BANK.RENTDETAIL.PAYTIME',
        type: 0,
        value: Utils.timetransTime(recordDetail.create_time),
      },
      {
        id: 'BANK.RENTDETAIL.EXPIRESTIME',
        type: 0,
        value: Utils.timetransTime(recordDetail.expire_time),
      },
    ];
    this.setState({
      recordDetail,
  ...
  render() {
    ...
    if (
      ...
      recordDetail.status !== 7 &&
      recordDetail.status !== 4
    ) {
      statusMessage = (
      ...
      statusMessage = <span className='validStatus'></span>;
    }
      ...
      <div className='BankDetailContainer'>
        ...
        <section className='detailContent' style={{ padding: '0 18px' }}>
          ...
          <div className='orderList'>
            ...
            <span className='orderStatus'>{recordDetail.orderId}</span>
          </div>
          <div className='orderAccount'>
            <div className='accountName'>
              <FormattedMessage id='BANK.RENTDETAIL.PAYACCOUNT' />
            </div>
            <div className='accountNum'>{recordDetail.pay_address}</div>
          </div>
          <div className='orderAccount'>
            <div className='accountName'>
              <FormattedMessage id='BANK.RENTDETAIL.TOACCOUNT' />
            </div>
            <div className='accountNum'>{recordDetail.energy_address}</div>
          </div>
