...
class TransactionsController extends React.Component {
  ...
  async componentDidMount() {
    ...
    console.log('accounts.selectedToken : ', accounts.selectedToken);
  ...
  render() {
    ...
    const { accounts, onCancel, prices, chains } = this.props;
        ...
        <div className='greyModal'>
          <div
            ...
            style={
              isTop
                ...
                : {
                    overflow:
                      id === CONTRACT_ADDRESS.USDT ? 'visible' : 'hidden',
                    height:
                      id === '_' ||
          ...
          >
            ...
            {id === '_' ? (
              <div className='desc trx'>
                <div className='cell'>
                  <div className='row1'>{balance}</div>
                  <div className='row2'>
                    <FormattedMessage id='TRANSACTION.TOKEN_INFO.AVAILABLE_BALANCE' />
                  </div>
                </div>
                <div className='cell'>
                  <div className='row1'>{frozenBalance}</div>
                  <div className='row2'>
              ...
              id === CONTRACT_ADDRESS.USDT && airdropInfo.isShow ? (
                <div className='desc usdt'>
                  <div
                    className='usdt_inner'
                    onClick={() =>
                      PopupAPI.changeState(APP_STATE.USDT_INCOME_RECORD)
                    }
                  >
                      ...
                      <div className='cell'>
                        <div className='income'>
                          ...
                          <div className='number'>
                            +
                            {new BigNumber(
                              new BigNumber(airdropInfo.yesterdayEarnings)
                                .shiftedBy(-6)
                                .toFixed(2)
                            ).toFormat()}
                          </div>
                      ...
                      <div className='cell'>
                        <div className='income'>
                          ...
                          <div className='number'>
                            +
                            {new BigNumber(
                              new BigNumber(airdropInfo.totalEarnings)
                                .shiftedBy(-6)
                                .toFixed(2)
                            ).toFormat()}
                          </div>
          ...
          <div className='tabNav'>
            <div
              ...
              onClick={async () => {
                ...
                Toast.loading('', 0);
                const transactions = await PopupAPI.getTransactionsByTokenId(
                  id,
                  '',
                  'all'
                );
          ...
          <div
            ...
            onScroll={async (e) => {
              ...
              if (transactions.records && transactions.records.length > 8) {
                const isTop = e.target.scrollTop === 0 ? false : true;
                console.log('isTop : ', isTop);
                console.log('e.target : ', e.target.scrollTop);
                console.log(
                  '========== ',
                  58 * transactions.records.length + 36 - 484
                );
                this.setState({ isTop });
                if (
                  e.target.scrollTop ===
                  ...
                  if (!isRequest) {
                    ...
                    Toast.loading('', 0);
                    const records = await PopupAPI.getTransactionsByTokenId(
                      id,
                      typeof transactions.finger === 'string'
                        ? transactions.finger
                        : ++transactions.finger,
                      key
                    );
                    ...
                    if (records.records.length === 0 || !records.finger)
                      ...
                      transactions.records = transactions.records.concat(
                        records.records
                      );
          ...
          >
            {transactions.records && transactions.records.length > 0 ? (
              <div className='lists'>
                {transactions.records.map((v, transIndex) => {
                  debugger;
                  const direction =
                    v.toAddress === v.fromAddress
                      ? 'send'
                      : v.toAddress === address
                      ? 'receive'
                      : 'send';
                  const addr =
                    v.toAddress === address ? v.fromAddress : v.toAddress;
                    ...
                    >
                      <div className='left'>
                        <div className='address'>
                          {addr
                            ? `${addr.substr(0, 4)}...${addr.substr(-12)}`
                            : v.type}
                        </div>
                        <div className='time'>
                          {moment(v.timestamp).format('YYYY-MM-DD HH:mm:ss')}
                        </div>
                      </div>
                      <div className='right'>
                        {addr
