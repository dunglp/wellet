        ...
        {recordList.map((val, key) => {
          ...
          return (
            <div
              ...
              onClick={() => {
                this.toMoreDetail(val.id);
              }}
            >
              <div className='address'>
                <img
                  src={require('../../../../assets/images/new/tronBank/receive.svg')}
                  alt='receive'
                />
                <span>{`${val.energy_address.substr(
                  0,
                  8
                )}...${val.energy_address.substr(-8)}`}</span>
              </div>
              <div className='recordCont'>
                <section className='recordLeftInfo'>
                  <div>
                    <FormattedMessage id='BANK.RENTRECORD.RENTDETAIL' />
                    {val.freeze_amount / Math.pow(10, 6)}WELUPS*{val.days}
                    <FormattedMessage id='BANK.RENTRECORD.TIMEUNIT' />
                  </div>
                  <div style={{ padding: '4px 0' }}>
                    <FormattedMessage id='BANK.RENTRECORD.DEADLINE' />
                    {Utils.timetransTime(val.expire_time)}
                  </div>
                  <div className='time'>
                    {Utils.timeFormatTime(val.create_time)}
                  </div>
                </section>
                <section className='recordRightInfo'>
                  <div className='cost'>
                    <FormattedMessage id='BANK.RENTRECORD.COST' />
                    {String(payMonkey).indexOf('.') > -1
                      ? payMonkey.toFixed(2)
                      : payMonkey}
                    WELUPS
                  </div>
                  <div className='recordValStatus'>{statusMessage}</div>
                </section>
              </div>
            </div>
          );
