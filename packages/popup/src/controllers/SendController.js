...
class SendController extends React.Component {
	...
	async changeAccount(address, e) {
		...
		const selectedToken = {
			...
			name: 'WEL',
			decimals: 6,
			amount: new BigNumber(accounts[address].balance)
				.shiftedBy(-6)
				.toString(),
		...
		this.setState(
			{ isOpen, selectedToken, selectedAddress: address },
			() => {
				this.validateAmount();
			}
		);
	...
	async onRecipientChange(address) {
		...
		if (!TronWeb.isAddress(address)) {
			...
			if (
				!account[chains.selected === '_' ? 'mainchain' : 'sidechain']
					.address
			) {
	...
	validateAmount() {
		...
		if (!this.state.recipient.isActivated) {
			if (
				(id === '_' &&
					value.gt(
						new BigNumber(selected.balance).shiftedBy(-6).minus(0.1)
					)) ||
				(id !== '_' &&
					new BigNumber(selected.balance)
						.shiftedBy(-6)
						.lt(new BigNumber(0.1)))
			) {
				return this.setState({
					amount: {
						...amount,
	...
	onSend() {
		...
		if (selected.type !== ACCOUNT_TYPE.LEDGER) {
			...
			const iframe = document.querySelector('#tronLedgerBridge')
				.contentWindow;
					...
					>
						...
						<div className='input'>
							<input
								type='text'
								onChange={(e) =>
									this.onRecipientChange(e.target.value)
								}
							/>
						</div>
					...
					>
						...
						<div className='input'>
							<input
								...
								onChange={(e) => {
									if (
										e.target.value != selectedToken.amount
									) {
										this.refs.max.classList.remove(
											'selected'
										);
									} else
