...
class SendController extends React.Component {
    ...
    if (event.data.target === 'LEDGER-IFRAME') {
      ...
      if (event.data.success) {
        ...
        if (event.data.error === 'User has not unlocked wallet')
          ...
          event.data.error === "Cannot read property 'message' of undefined"
        )
        ...
        Toast.fail(
          id ? formatMessage({ id }) : event.data.error,
          3,
          () => {
            PopupAPI.setGaEvent(
              'Ledger',
  ...
  async changeAccount(address, e) {
    ...
    const selectedToken = {
      ...
      balance: new BigNumber(
        accounts[address].balance - accounts[address].frozenBalance
      )
        .shiftedBy(-6)
        .toString(),
      frozenBalance: new BigNumber(accounts[address].frozenBalance)
        .shiftedBy(-6)
        .toString(),
    };
    ...
    this.setState({ isOpen, selectedToken, selectedAddress: address }, () => {
      this.validateAmount();
    });
  ...
  async onRecipientChange(address) {
    ...
    if (!address.length) {
      return this.setState({
        recipient: { value: '', valid: false, error: '' },
      });
    }
    ...
    if (!TronWeb.isAddress(address)) {
      ...
      if (
        !account[chains.selected === '_' ? 'mainchain' : 'sidechain'].address
      ) {
  ...
  validateAmount() {
    ...
    if (!this.state.recipient.isActivated) {
      if (
        (id === '_' &&
          value.gt(new BigNumber(selected.balance).shiftedBy(-6).minus(0.1))) ||
        (id !== '_' &&
          new BigNumber(selected.balance).shiftedBy(-6).lt(new BigNumber(0.1)))
      ) {
        return this.setState({
          amount: {
            ...amount,
      ...
      if (
        ...
        value.gt(new BigNumber(selected.balance).shiftedBy(-6).minus(1))
      ) {
        return this.setState({
          amount: {
            ...amount,
    ...
    if (id.match(/^W/)) {
      ...
      if (valid) {
        const isEnough = new BigNumber(selected.balance)
          .shiftedBy(-6)
          .gte(new BigNumber(1))
          ? true
          : false;
      ...
      return this.setState({
        amount: {
          ...
          valid:
            id === '_'
              ? value.lte(
                  new BigNumber(selected.balance).shiftedBy(-6).minus(1)
                )
              : new BigNumber(selected.balance)
                  .shiftedBy(-6)
                  .gte(new BigNumber(1)), //new BigNumber(selected.balance).shiftedBy(-6).gte(new BigNumber(1)) ? true : false,
          error: 'EXCEPTION.SEND.BANDWIDTH_NOT_ENOUGH_ERROR',
        },
      });
    }
  ...
  onSend() {
    ...
    const { id, decimals, name } = this.state.selectedToken;
        ...
        iframe.postMessage(
          {
            ...
            data: {
              toAddress,
              fromAddress,
              amount: new BigNumber(amount).shiftedBy(6).toString(),
            },
          },
          '*'
        );
  ...
  onCancel() {
    ...
    } else PopupAPI.changeState(APP_STATE.READY);
  }
  ...
  handleClose() {
    ...
    const iframe = document.querySelector('#tronLedgerBridge').contentWindow;
    iframe.postMessage(
      { target: 'LEDGER-IFRAME', action: 'cancel transaction', data: {} },
      '*'
    );
    this.setState({ loadingLedger: false, loading: false }, () => {
      Toast.fail(
        formatMessage({ id: 'CREATION.LEDGER.TIP_CANCEL_TRANSACTION' }),
        3,
        () => {},
        true
      );
  ...
  render() {
    ...
    tokens = Utils.dataLetterSort(
      Object.entries(tokens)
        .filter(
          ([tokenId, token]) =>
            (typeof token === 'object' && !token.hasOwnProperty('chain')) ||
    ...
    return (
      ...
      >
        ...
        <div className='pageHeader'>
          <div className='back' onClick={(e) => this.onCancel()}>
            &nbsp;
          </div>
        ...
        <div className='greyModal'>
          <div className='input-group'>
            ...
            >
              ...
              <div
                ...
                style={
                  isOpen.account
                    ? Object.entries(accounts).length <= 5
                      ? {
                          height: 36 * Object.entries(accounts).length,
                        }
                      : {
                          height: 180,
                          overflow: 'scroll',
              ...
              >
                {Object.entries(accounts).map(([address]) => (
                  <div
                    key={address}
          ...
          <div className={`input-group${recipient.error ? ' error' : ''}`}>
            ...
            <div className='tipError'>
              {recipient.error ? (
                <FormattedMessage id={recipient.error} />
              ) : null}
            </div>
          </div>
          <div className='input-group'>
          ...
          <Button
            id='ACCOUNT.SEND'
            isLoading={loading}
            isValid={amount.valid && recipient.valid}
            onClick={() => this.onSend()}
          />
        </div>
      </div>
