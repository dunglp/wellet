...
class RegistrationController extends React.Component {
	...
	onPasswordChange(value) {
		...
		const hasSpecial = /[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?\d]+/.test(
			trimmed
		);
		const showCriteria = trimmed.length ? true : false;
		let isValid = trimmed.length
			? VALIDATION_STATE.INVALID
			: VALIDATION_STATE.NONE;

		if (hasLength && hasSpecial) isValid = VALIDATION_STATE.VALID;
	...
	onButtonClick() {
		...
		PopupAPI.setPassword(password.value)
			.catch((error) =>
				this.setState({
					error,
				})
					...
					<div className={"language " + fliterLanguage.key}>
						...
						<div className="drop">
							{languages.map(({ key, name }) => (
								<div
									onClick={() => PopupAPI.setLanguage(key)}
									className={"item " + key}
								>
									{name}
								</div>
				...
				{error ? (
					<div className="errorModal hasBottomMargin">
						<FormattedMessage
							...
							id="ERRORS.ACCOUNT_CREATION_FAILED"
						/>
				...
				<div className="greyModal registrationModel">
					<div className="inputGroup">
						...
						{password.showCriteria ? (
							<div className="criteria">
								<InputCriteria
									id="PASSWORD_CRITERIA.HAS_LENGTH"
									isValid={password.hasLength}
								/>
								<InputCriteria
									id="PASSWORD_CRITERIA.HAS_SPECIAL"
					...
					<div className="inputGroup">
						...
						{repeatPassword.showCriteria ? (
							<div className="criteria">
								<InputCriteria
									id="PASSWORD_CRITERIA.NO_REPEAT"
									isValid={!repeatPassword.showCriteria}
								/>
							</div>
						) : null}
					</div>
