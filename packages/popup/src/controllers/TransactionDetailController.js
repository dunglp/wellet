...
class TransactionDetailController extends React.Component {
	...
	copy(id) {
		...
		ids.forEach((v) => {
			document.getElementById(v).innerText = formatMessage({
				id: 'TRANSACTION_DETAIL.ENABLE_COPY',
			});
		});
		document.getElementById(id).innerText = formatMessage({
			id: 'TRANSACTION_DETAIL.HAVE_COPIED',
		});
	}
	...
	render() {
		...
		let amount =
			t.contractType === 31
				? t.tokenTransferInfo
					? t.tokenTransferInfo.amount_str
					: t.trigger_info.call_value
				: t.contractData.amount;
		amount = new BigNumber(amount)
			.shiftedBy(-selectedToken.decimals)
			.toString();
		return (
			<div
				className='insetContainer transactions'
				onClick={() => {
			...
			>
				...
				<div className='greyModal detail'>
					<div className='part1'>
						...
						<div className='state'>
							<FormattedMessage id='TRANSACTION_DETAIL.TRANSFER_SUCCESS' />
						</div>
						<div className='amount'>
							{t.toAddress === address ? '+' : '-'}
							{amount} {selectedToken.abbr}
						</div>
					</div>
					<div className='part2'>
						<div className='cell'>
							...
							<CopyToClipboard
								text={t.ownerAddress}
								onCopy={(e) => this.copy('ownerAddress')}
							>
								<div className='content'>
									<a
										data-tip={formatMessage({
											id:
												'TRANSACTION_DETAIL.ENABLE_COPY',
										})}
										data-for='ownerAddress'
									>
										{t.ownerAddress}
									</a>
									<ReactTooltip
						...
						<div className='cell'>
							...
							<CopyToClipboard
								text={t.block}
								onCopy={(e) => this.copy('block_height')}
							>
								<div className='content'>
									<a
										data-tip={formatMessage({
						...
						<div className='cell'>
							...
							<CopyToClipboard
								text={t.block}
								onCopy={(e) => this.copy('block_confirmed')}
							>
								<div className='content'>
									<a
										data-tip={formatMessage({
										...
										data-for='block_confirmed'
									>
										{t.tokenTransferInfo.num_of_blocks -
											t.tokenTransferInfo.ref_block_num}
									</a>
									<ReactTooltip
										id='block_confirmed'
						...
						{t.cost.energy_fee >= 0 &&
						t.cost.net_fee >= 0 &&
						t.cost.energy_fee + t.cost.net_fee > 0 ? (
							<div className='cell'>
								<div className='title'>
									...
									<Popover
										...
										overlay={
											<div style={{ padding: 10 }}>
												{' '}
												<FormattedMessage id='TRANSACTION_DETAIL.EXPLAIN.CONSUME' />{' '}
											</div>
										}
										placement='right'
										align={{
								...
								<div className='content'>
									{new BigNumber(
										t.cost.energy_fee + t.cost.net_fee
									)
										.shiftedBy(-6)
										.toString()}{' '}
									WEL
					...
					<div
						className='part3'
						onClick={() =>
							window.open(`${WELSCAN}/transaction/${t.hash}`)
						}
					>
