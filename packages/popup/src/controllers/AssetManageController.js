...
class AssetManageController extends React.Component {
  ...
  renderDeleteToken(tokens, deleteToken) {
    if (deleteToken.show) {
      ...
      const { imgUrl = false, name, abbr = false, symbol = false } = tokens[
        field
      ][deleteToken.tokenId];
      return (
        <div className='deleteTokenWrap'>
          <div className='deleteToken'>
            ...
            <div className='btnGroup'>
              <button
                className='cancel'
                onClick={() => this.cancelDeleteToken()}
              >
                <FormattedMessage id='ASSET.CONFIRM.CANCEL' />
              </button>
              <button
                className='confirm'
                onClick={() => this.confirmDeleteToken()}
              >
  ...
  confirmDeleteToken() {
    ...
    const fs = filterTokens.map(({ tokenId: id, ...token }) => {
      if (id === tokenId) token.isList = false;
      return { tokenId: id, ...token };
    });
    this.setState({
      filterTokens: fs,
      deleteToken: { show: false, tokenId: '' },
    });
  ...
  render() {
    ...
    const trx = {
      ...
      balance:
        selected.balance +
        (selected.frozenBalance ? selected.frozenBalance : 0),
      abbr: 'WELUPS',
      decimals: 6,
      imgUrl: trxImg,
      price: trx_price,
    ...
    tokens = Utils.dataLetterSort(
      Object.entries(tokens)
        ...
        .filter((v) => v.balance > 0 || (v.balance == 0 && !v.isLocked)),
      'abbr',
      'symbol',
      topArray
    );
    ...
    tokens = tokens
      ...
      .map(({ tokenId, ...token }) => {
        if (
          TOP_TOKEN[
            chains.selected === '_' ? 'mainchain' : 'sidechain'
          ].includes(tokenId) ||
          tokenId === '_'
        )
          token.isTop = true;
          ...
          <div className={`input${address.value === '' ? '' : ' hasValue'}`}>
            <input
              ...
              onChange={async (e) => {
                ...
                if (value !== '') {
                  if (TronWeb.isAddress(value)) {
                    ...
                    if (!fTokens.length) {
                      ...
                      fTokens[0].price = selected.tokens.smart.hasOwnProperty(
                        value
                      )
                        ? selected.tokens.smart[value].price
                        : 0;
                    }
                    fTokens[0].isList =
                      selected.tokens.smart.hasOwnProperty(value) &&
                      (!selected.tokens.smart[value].hasOwnProperty(
                        'isLocked'
                      ) ||
                        !selected.tokens.smart[value].isLocked)
                        ? true
                        : false;
                  } else {
                    let topArray2 = [];
                    ...
                    fTokens = allTokens
                      ...
                      .filter(
                        ({ name, abbr }) =>
                          name.match(regexp) || abbr.match(regexp)
                      );
                    fTokens = fTokens.map(({ tokenId, ...token }) => {
                      const field = tokenId.match(/^W/) ? 'smart' : 'basic';
                      const name = token.name.match(regexp)
                        ? token.name
                            .split(token.name.match(regexp)[0])
                            .join(`<i>${token.name.match(regexp)[0]}</i>`)
                        : token.name;
                      const abbr = token.abbr.match(regexp)
                        ? token.abbr
                            .split(token.abbr.match(regexp)[0])
                            .join(`<i>${token.abbr.match(regexp)[0]}</i>`)
                        : token.abbr;
                      token.isList =
                        selected.tokens[field].hasOwnProperty(tokenId) &&
                        (!selected.tokens[field][tokenId].hasOwnProperty(
                          'isLocked'
                        ) ||
                          !selected.tokens[field][tokenId].isLocked)
                          ? true
                          : false;
                      token.html = `${name}(${abbr})`;
                      token.balance = selected.tokens[field].hasOwnProperty(
                        tokenId
                      )
                        ? selected.tokens[field][tokenId].balance
                        : 0;
                      token.price = selected.tokens[field].hasOwnProperty(
                        tokenId
                      )
                        ? selected.tokens[field][tokenId].price
                        : 0;
                      token.isTop = TOP_TOKEN[
                        chains.selected === '_' ? 'mainchain' : 'sidechain'
                      ].includes(tokenId);
                ...
                this.setState({
                  filterTokens: fTokens,
                  address: { value },
                });
              }}
              placeholder={formatMessage({
                id: 'ASSET.ADD_TOKEN.PLACEHOLDER',
              })}
            />
          </div>
              ...
              {filterTokens.map(
                ({
                  ...
                  return (
                    <div className='cell'>
                      <img
                        src={imgUrl ? imgUrl : token10DefaultImg}
                        onError={(e) => {
                      ...
                      <div className='desc'>
                        <div
                          className={`row1${isVerify ? ' isVerify' : ''}`}
                          dangerouslySetInnerHTML={{
                            __html: html,
                          }}
                        ></div>
                        <div className='row2'>
                          {tokenId.match(/^W/) ? 'contract' : 'ID'} :{' '}
                          {tokenId.match(/^W/)
                            ? `${tokenId.substr(0, 7)}...${tokenId.substr(-7)}`
                            : tokenId}
                        </div>
                      </div>
                      {isTop ? null : (
                        <Switch
                          ...
                          checked={isList}
                          onClick={(e) => {
                            const field = tokenId.match(/^W/)
                              ? 'smart'
                              : 'basic';
                            const filters = filterTokens.map(
                              ({ tokenId: id, ...token }) => {
                                if (id === tokenId) token.isList = !isList;
                                return {
                                  tokenId: id,
                                  ...token,
                                };
                    ...
                    {!isTop ? (
                      <Switch
                        ...
                        onClick={(e) => {
                          const field = tokenId.match(/^W/) ? 'smart' : 'basic';
                          if (balance > 0) {
                            selected.tokens[field][
                              tokenId
                            ].isLocked = !isLocked;
                            if (filterTokens.length) {
                              const fs = filterTokens.map(
                                ({ tokenId: id, ...token }) => {
                                  if (id === tokenId) token.isList = isLocked;
