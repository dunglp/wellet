...
class AccountName extends React.Component {
	...
	onChange(name) {
		...
		if (/^\s$/.test(name) || !trimmed.length) return this.setState(state);

		if (trimmed.trim().length >= 4) state.hasLength = true;

		if (/^([A-Za-z\d\s])+$/.test(trimmed)) state.isAlphanumeric = true;

		if (
			!Object.values(accounts).some(({ name }) => name === trimmed.trim())
		)
			state.isUnique = true;
					...
					<div className="inputGroup hasBottomMargin">
						...
						{showCriteria ? (
							<div className="criteria">
								<InputCriteria
									id="CREATION_CRITERIA.HAS_LENGTH"
									isValid={hasLength}
								/>
								<InputCriteria
									id="CREATION_CRITERIA.IS_ALPHANUMERIC"
									isValid={isAlphanumeric}
								/>
								<InputCriteria
									id="CREATION_CRITERIA.IS_UNIQUE"
									isValid={isUnique}
								/>
							</div>
						) : null}
