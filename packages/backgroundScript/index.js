...
const backgroundScript = {
  ...
  bindPopupDuplex() {
    ...
    duplex.on('popup:connect', () => this.walletService.startPolling());

    duplex.on('popup:disconnect', () => this.walletService.stopPolling());
    ...
    duplex.on('requestState', ({ resolve }) =>
      resolve(this.walletService.state)
    );
    ...
    duplex.on('unlockExport', this.walletService.unlockExport);
    ...
    duplex.on(
      ...
      this.walletService.getTransactionsByTokenId
    );
    ...
    duplex.on(
      ...
      this.walletService.setSelectedBankRecordId
    );
    duplex.on(
      'changeDealCurrencyPage',
      this.walletService.changeDealCurrencyPage
    );
    ...
    duplex.on(
      ...
      this.walletService.setLedgerImportAddress
    );
    duplex.on(
      'getLedgerImportAddress',
      this.walletService.getLedgerImportAddress
    );
  ...
  bindTabDuplex() {
    duplex.on(
      'tabRequest',
      async ({ hostname, resolve, data: { action, data, uuid } }) => {
        ...
        switch (action) {
          case 'init': {
            ...
            if (StorageService.ready) {
              ...
              const { address, name, type } = this.walletService.accounts[
          ...
          case 'sign': {
            ...
            try {
              const { transaction, input } = data;

              const { selectedAccount } = this.walletService;

              const tronWeb = NodeService.tronWeb;
              const account = this.walletService.getAccount(selectedAccount);
              const appWhitelist = this.walletService.appWhitelist.hasOwnProperty(
                hostname
              )
                ? this.walletService.appWhitelist[hostname]
                : {};
              ...
              if (typeof input === 'string') {
                ...
                if (
                  appWhitelist &&
                  (duration === -1 || duration >= Date.now())
                ) {
                  logger.info(
                    'Automatically signing transaction',
                    signedTransaction
                  );
                  return resolve({
                    success: true,
              ...
              const { mapped, error } = await transactionBuilder(
                tronWeb,
                contractType,
                input
              ...
              const signedTransaction = await account.sign(
                ...
                NodeService._selectedChain === '_'
                  ? NodeService.sunWeb.mainchain
                  : NodeService.sunWeb.sidechain
              );

              const whitelist = this.walletService.contractWhitelist[
                input.contract_address
              ];
              ...
              if (
                ...
                authorizeDapps.hasOwnProperty(contractAddress)
              ) {
                logger.info(
                  'Automatically signing transaction',
                  signedTransaction
          ...
          case 'setVisited': {
            ...
            if (href) {
              this.walletService.phishingList = phishingList.map(
                ({ url, isVisit }) => {
  ...
  bindWalletEvents() {
    ...
    this.walletService.on('setNode', (node) => BackgroundAPI.setNode(node));

    this.walletService.on('setChain', (chain) => BackgroundAPI.setChain(chain));
