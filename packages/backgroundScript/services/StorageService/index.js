...
const StorageService = {
  ...
  async unlockExport(password) {
    if (!(await this.dataExists())) return 'ERRORS.NOT_SETUP';

    if (!this.ready) {
      logger.error('Attempted to decrypt data whilst already unencrypted');
      return 'ERRORS.NOT_UNLOCKED';
    }
  ...
  migrate() {
    try {
      ...
      const decrypted = Utils.decrypt(JSON.parse(storage), this.password);

      const { accounts, currentAccount } = decrypted;
  ...
  removePendingTransaction(address, txID) {
    ...
    this.pendingTransactions[address] = this.pendingTransactions[
      address
  ...
  async getDappList(isFromStorage) {
    ...
    if (!isFromStorage) {
      const {
        ...
        .get('https://list.tronlink.org/dapphouseapp/plug')
        .catch((e) => {
  ...
  purge() {
    logger.warn(
      'Purging TronLink. This will remove all stored transaction data'
    );
