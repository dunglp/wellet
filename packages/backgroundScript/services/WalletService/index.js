...
class Wallet extends EventEmitter {
  ...
  migrate(password) {
    ...
    localStorage.setItem(
      'TronLink_WALLET.bak',
      localStorage.getItem('TronLink_WALLET')
    );
    localStorage.removeItem('TronLink_WALLET');

    accounts.forEach((account) => this.importAccount(account));
  ...
  async _pollAccounts() {
    ...
    if (accounts.length > 0) {
      ...
      const {
        data: {
          data: { result: smartTokenPriceList },
        },
      } = await axios
        .get(`${WELSCAN_API}/tokenrecords?page=1&limit=4000`)
        .catch((e) => {
      ...
      smartPrice = smartTokenPriceList.filter((tok /*{token_record_type}*/) => {
        ...
        return tok.token_record_type.toUpperCase() === 'WRC20';
      });
      ...
      for (const account of accounts) {
        if (account.address === this.selectedAccount) {
          Promise.all([account.update(basicPrice, smartPrice /*, usdtPrice*/)])
            .then(() => {
              if (account.address === this.selectedAccount)
        ...
        } else await account.update(basicPrice, smartPrice /*, usdtPrice*/);
      }
  ...
  async _updatePrice() {
    if (!StorageService.ready) return;
    ...
    const prices = axios(
      'https://price-api.crypto.com/price/v1/exchange/welups-blockchain'
    );
    Promise.all([prices])
      .then((res) => {
  ...
  async _openPopup() {
    ...
    if (this.popup && (await this._updateWindow())) return;
    ...
    for (const account of accounts) {
      if (account.address === this.selectedAccount) {
        const r = await account
          .update(basicPrice, smartPrice, usdtPrice)
          .catch((e) => false);
        if (r) {
          res = true;
          this.emit('setAccount', this.selectedAccount);
  ...
  async resetState() {
    ...
    if (
      this.state === APP_STATE.REQUESTING_CONFIRMATION &&
  ...
  async unlockWallet(password) {
    ...
    if (StorageService.needsMigrating) {
      ...
      if (!success) return Promise.reject('ERRORS.INVALID_PASSWORD');
    ...
    Object.keys(this.accounts).forEach((address) => {
      ...
      if (!tokens.hasOwnProperty(oldAddress)) return;
    ...
    if (this.confirmations.length === 0) {
      await this.setCache();
      this._setState(APP_STATE.READY);
    } else this._setState(APP_STATE.REQUESTING_CONFIRMATION);
  }
  ...
  whitelistContract(confirmation, duration) {
    const {
      input: { contract_address: address },
      contractType,
      hostname,
    } = confirmation;
    //if(!address)
    ...
    if (contractType !== 'TriggerSmartContract') {
      ...
      if (!this.appWhitelist[hostname]) this.appWhitelist[hostname] = {};

      this.appWhitelist[hostname].duration =
        duration === -1 ? -1 : Date.now() + duration;
      logger.info(
        `Added auto sign on host ${hostname} with duration ${duration} to whitelist`
      );
      ...
      logger.info(
        `Added contact ${address} on host ${hostname} with duration ${duration} to whitelist`
      );

      ga('send', 'event', {
        eventCategory: 'Smart Contract',
        eventAction: 'Whitelisted Smart Contract',
        eventLabel: TronWeb.address.fromHex(
          confirmation.input.contract_address
        ),
        eventValue: duration,
  ...
  acceptConfirmation(whitelistDuration) {
    if (!this.confirmations.length) return Promise.reject('NO_CONFIRMATIONS');

    if (this.isConfirming) return Promise.reject('ALREADY_CONFIRMING');
  ...
  async addAccount({ mnemonic, name }) {
    ...
    if (Object.keys(this.accounts).length === 0) this.setCache();

    const account = new Account(ACCOUNT_TYPE.MNEMONIC, mnemonic);
  ...
  async importAccount({ privateKey, name }) {
    ...
    const account = new Account(
      privateKey.match(/^W/) && TronWeb.isAddress(privateKey)
        ? ACCOUNT_TYPE.LEDGER
  ...
  async setCache(isResetPhishingList = true) {
    ...
    Promise.all([allTokens /*, trc20tokens_s*/]).then((res) => {
      const t = [];
      const t2 = [];
      res[0].data.data.result.forEach(
        ({
          token_abbreviation,
          token_name,
          token_icon = false,
          ...
          t.push({
            ...
            abbr: token_abbreviation,
            name: token_name,
            imgUrl: token_icon,
            decimals: precision,
            isBlack:
              reputation === 'OK' || reputation === 'Neutral' ? false : true,
          });
        }
      );
  ...
  async selectNode(nodeID) {
    ...
    Object.values(this.accounts).forEach((account) => account.reset());
  ...
  async selectChain(chainId) {
    if (StorageService.chains.selectedChain !== chainId) {
      ...
      const node = Object.entries(nodes.nodes).filter(
        ([nodeId, node]) => node.chain === chainId && node.default
      )[0];
  ...
  getAccounts(sideChain = false) {
    const accounts = Object.entries(this.accounts).reduce(
      (accounts, [address, account]) => {
        if (sideChain && account.type === ACCOUNT_TYPE.LEDGER) return;
        ...
        accounts[address] = {
          ...
          tokenCount:
            Object.keys(account.tokens.basic).length +
            Object.keys(account.tokens.smart).length,
          asset: account.asset,
          type: account.type,
  ...
  getSelectedToken() {
    return JSON.stringify(StorageService.selectedToken) === '{}'
      ? { id: '_', name: 'WELUPS', abbr: 'WELUPS', amount: 0, decimals: 6 }
  ...
  async addSmartToken(token) {
    const { selectedAccount: address } = this;
  ...
  async sendTrx({ recipient, amount }) {
    return await this.accounts[this.selectedAccount].sendTrx(recipient, amount);
  }
  ...
  async rentEnergy({ _freezeAmount, _payAmount, _days, _energyAddress }) {
    ...
    try {
      ...
      const contractInstance = await NodeService.tronWeb
        .contract()
        .at(bankContractAddress);
      const result = await contractInstance
        .entrustOrder(_freezeAmount, _days, _energyAddress)
        .send(
          {
  ...
  async bankOrderNotice({ energyAddress, trxHash, requestUrl }) {
    const { data: isValid } = await axios
      .post(requestUrl, { receiver_address: energyAddress, trxHash })
      .then((res) => res.data)
      .catch((err) => {
        logger.error(err);
      });
    if (!isValid) return logger.warn('Failed to get bank order data');
    return isValid;
  }
  ...
  async getBankDefaultData({ requestUrl }) {
    ...
    if (!defaultData) return logger.warn('Failed to get default data');
    return defaultData;
  }
  ...
  async isValidOverTotal({ receiverAddress, freezeAmount, requestUrl }) {
    const { data: isValid } = await axios
      .get(requestUrl, {
        params: { receiver_address: receiverAddress, freezeAmount },
      })
      .then((res) => res.data)
      .catch((err) => {
        logger.error(err);
    ...
    let isValidVal = 0;
    if (isValid) isValidVal = 0;
    else isValidVal = 1;
    return isValidVal;
  }

  async calculateRentCost({ receiverAddress, freezeAmount, days, requestUrl }) {
    const { data: calculateData } = await axios
      .get(requestUrl, {
        params: { receiver_address: receiverAddress, freezeAmount, days },
      })
      .then((res) => res.data)
      .catch((err) => {
        logger.error(err);
      });
    if (!calculateData) return logger.warn('Failed to get payMount data');
    return calculateData;
  }

  async isValidOrderAddress({ address, requestUrl }) {
    const { data: isRentData } = await axios
      .get(requestUrl, { params: { receiver_address: address } })
      .then((res) => res.data)
      .catch((err) => {
        logger.error(err);
      });
  ...
  async getBankRecordList({ address, limit, start, type, requestUrl }) {
    const {
      ...
      params: { receiver_address: address, limit, start, type },
    });
    if (!recordData) return logger.warn('Failed to get bank record data');
    return recordData;
  }
  ...
  async getBankRecordDetail({ id, requestUrl }) {
    const { data: bankRecordDetail } = await axios
      .get(requestUrl, { params: { id } })
      .then((res) => res.data)
      .catch((err) => {
        logger.error(err);
      });
  ...
  changeDealCurrencyPage(status) {
    // change deal currency page status
  ...
  exportAccount() {
    const { mnemonic, privateKey } = this.accounts[this.selectedAccount];
  ...
  async getTransactionsByTokenId({
    ...
    params.start = 0;
    params.end = 0;
    params.num = 1 + parseInt(finger);
    params.sort = 'desc';
    params.limit = limit;
    if (!tokenId.match(/^W/) && tokenId === '_')
      requestUrl += `/address/transactions/${address}`;
    else requestUrl += `/contract/transactions/${tokenId}`;

    logger.info('AAAAAAAAAAAAAAAAAAA');
    logger.debug('[getTransbyTokenId] requestURL: ', requestUrl);

    console.log('[getTransbyTokenId] requestURL: ', requestUrl);
    const transFilter = ({
      contract: {
        parameter: {
          raw: { name, owner_address: fromAddress, to_address: toAddress },
        },
        type,
      },
    }) => {
      console.log(
        `tokenId : ${tokenId} ------ name : ${name} ------ type : ${type}`
      );
      //   if (type === 'TriggerSmartContract' || type === 'CreateSmartContract')
      //     return false;
    ...
    const {
      data: {
        data: { result: records, from, to, total },
      },
    } = res;

    logger.debug(
      `[getTransactionsByTokenId] Token ${tokenId} transactions record: `,
      records
    );

    console.log('[getTransactionsByTokenId] fetched result: ', res);

    if (!records) return { records: [], finger, from, to, total };

    newRecord = records.filter(transFilter).map(
      ...
      ({
        ...
        contract: {
          parameter: {
            raw: { owner_address: fromAddress, to_address: toAddress, amount },
          },
          type,
        },
      }) => {
    ...
    logger.debug(
      '[getTransactionsByTokenId] Processed transactions record: ',
      newRecord
    );

    return { records: newRecord, finger, from, to, total };
  }

  async getNews() {
    const apiUrl = API_URL;
    const res = await axios
      .get(`${apiUrl}/api/activity/announcement/reveal_v2`)
      .catch((e) => false);
  ...
  async addCount(id) {
    const apiUrl = API_URL;
    const res = await axios
      .post(`${apiUrl}/api/activity/announcement/pv`, { id })
      .catch((e) => false);
    if (res && res.data.code === 0) return true;
  ...
  async setAirdropInfo(address) {
    ...
    const res = await axios
      .get(`${apiUrl}/api/wallet/airdrop_transaction`, {
  ...
  getAllDapps() {
    return StorageService.hasOwnProperty('allDapps')
      ? StorageService.allDapps
      : [];
  }
  ...
  getAllTokens(selectedChain = '_') {
    return StorageService.hasOwnProperty('allTokens')
      ? selectedChain === '_'
        ? StorageService.allTokens.mainchain
        : StorageService.allTokens.sidechain
      : {};
  }
  ...
  async setTransactionDetail(_hash) {
    ...
    const {
      data: { data: res },
    } = await axios.get(`${requestUrl}/transactions/${_hash}`).catch((e) => {
  ...
  getAuthorizeDapps() {
    return StorageService.hasOwnProperty('authorizeDapps')
      ? StorageService.authorizeDapps
      : {};
  }
  ...
  getLedgerImportAddress() {
    return this.hasOwnProperty('ledgerImportAddress')
      ? this.ledgerImportAddress
      : [];
  }
  ...
  getVTokenList() {
    return StorageService.hasOwnProperty('vTokenList')
      ? StorageService.vTokenList
      : [];
  }

  setPushMessage({
    iconUrl = 'packages/popup/static/icon.png',
    title,
    message,
    hash,
  }) {
    const timer = setInterval(async () => {
      ...
      const transaction = await NodeService.tronWeb.trx.getConfirmedTransaction(
        hash
      );
  ...
  async depositTrc20({ contract_address, amount }) {
    return await this.accounts[this.selectedAccount].depositTrc20(
      contract_address,
      amount
    );
  }
  ...
  async withdrawTrc20({ contract_address, amount }) {
    return await this.accounts[this.selectedAccount].withdrawTrc20(
      contract_address,
      amount
    );
  }
