...
import { decodeBase58Address } from './utils/crypto';
...
export default class TronWeb extends EventEmitter {
    ...
    return {
      ...
      fromPrivateKey(privateKey, strict = false) {
        try {
          ...
          const ret = utils.crypto.tronBase58toWel(
            utils.crypto.pkToAddress(privateKey, strict)
          );
  ...
  static sha3(string, prefix = true) {
    return (
      (prefix ? '0x' : '') +
      keccak256(Buffer.from(string, 'utf-8'))
        .toString()
        .substring(2)
    );
  }
  ...
  static fromAscii(string, padding) {
    ...
    return `0x${Buffer.from(string, 'ascii')
      .toString('hex')
      .padEnd(padding, '0')}`;
  }
