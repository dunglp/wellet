export default {
  ...
  isValidOverTotal(receiverAddress, freezeAmount, requestUrl) {
    return this.duplex.send('isValidOverTotal', {
      receiverAddress,
      freezeAmount,
      requestUrl,
    });
  ...
  getTransactionsByTokenId(tokenId, fingerprint, direction, limit) {
    ...
    return this.duplex.send('getTransactionsByTokenId', {
      tokenId,
      fingerprint,
      direction,
      limit,
  ...
  calculateRentCost(receiverAddress, freezeAmount, days, requestUrl) {
    return this.duplex.send('calculateRentCost', {
      receiverAddress,
      freezeAmount,
      days,
      requestUrl,
  ...
  getBankRecordList(address, limit, start, type, requestUrl) {
    return this.duplex.send('getBankRecordList', {
      address,
      limit,
      start,
      type,
  ...
  setGaEvent(eventCategory, eventAction, eventLabel, referrer = '') {
    this.duplex.send(
      'setGaEvent',
      { eventCategory, eventAction, eventLabel, referrer },
      false
    );
  },
  ...
  depositTrc20(contractAddress, amount) {
    return this.duplex.send('depositTrc20', { contractAddress, amount });
  },

  withdrawTrc20(contractAddress, amount) {
    return this.duplex.send('withdrawTrc20', { contractAddress, amount });
  },
};
