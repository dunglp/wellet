...
const Utils = {
  ...
  dataLetterSort(data, field, field2, topArray) {
    ...
    if (topArray) {
      ...
      data = data.filter(
        ({ tokenId }) => !topArray.map((v) => v.tokenId).includes(tokenId)
      );
    }
    const list = {};
    const LetterArray = [
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const letter = d
        ...
        .substr(0, 1)
        .toUpperCase();
      if (!list[letter]) list[letter] = [];

      list[letter].push(data[i]);
    }
    LetterArray.forEach((v) => {
      if (list[v]) needArray = needArray.concat(list[v]);
    });
    return needArray;
  },
  ...
  decodeParams(message, abiCode, function_selector) {
    ...
    if (cutArr[2] !== '') {
      ...
      return abi[0].inputs.map(({ name, type }, i) => {
        let value;
        const array = byteArray.filter(
          (v, index) => index >= 32 * i && index < 32 * (i + 1)
        );
        if (type === 'address') {
          value = TronWeb.address.fromHex(
            `41${TronWeb.utils.code.byteArray2hexStr(
              array.filter((v, i) => i > 11)
            )}`
          );
        } else if (type === 'trcToken') {
          value = TronWeb.toDecimal(
            `0x${TronWeb.utils.code.byteArray2hexStr(array)}`
          );
        } else {
          value = TronWeb.toDecimal(
            `0x${TronWeb.utils.code.byteArray2hexStr(array)}`
